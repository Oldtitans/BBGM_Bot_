    ...
    if len(text) == 1:
        ...
        if exportCheck:
            ...
            try: 
                ...
                print("here")
                if ".gz" in url:
                    b = os.path.getsize(f'exports/{message.guild.id}-export.gz')

                    if b > 30*1000*1000:
                        await message.channel.send("Too large of an export. Load aborted. export size is "+str(b/1000000)+" MB, allowed maximum (for .gz files) is 30 MB")
                        return
                    else:
                ...
                async with aiofiles.open(f'exports/{message.guild.id}-export.json', 'r') as f:
                    b = os.path.getsize(f'exports/{message.guild.id}-export.json')
                    if b > 300*1000*1000:
                        await message.channel.send("Too large of an export. Load aborted. export size is "+str(b/1000000)+" MB, allowed maximum is 300 MB")
                        return
                    else:
